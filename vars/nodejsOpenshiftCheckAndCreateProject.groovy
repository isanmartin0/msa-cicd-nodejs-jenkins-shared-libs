...
def call(body) {
    ...
    try {
        ...
        withCredentials([string(credentialsId: "${config.artifactoryNPMAuth}", variable: 'ARTIFACTORY_NPM_AUTH')]) {
            withCredentials([string(credentialsId: "${config.artifactoryNPMEmailAuth}", variable: 'ARTIFACTORY_NPM_EMAIL_AUTH')]) {
